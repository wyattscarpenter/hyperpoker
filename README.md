...
TODO: the rules and cards section are slowly merging, or convergently evolving, although the history and research sections are still distinct, which is really the important part.

This section concerns questions of how cards should be generalized, in addition to simple rules or numerical adjustments. Therefore, some things are marked as (arbitrary art need), where I predict some implemention of a generalization could (should?) require an infinite or arbitrarily-large amount of art effort. This probably could be dealt with using [procedural generation](https://en.wikipedia.org/wiki/Procedural_generation) of the relevant art assets, or perhaps with modern [AI art](https://en.wikipedia.org/wiki/Artificial_intelligence_visual_art) systems (although, in my experience, circa 2025, good luck trying to get the stock models to generate varied yet consistent things like face cards, especially of sufficient quality, complexity, and crispness) (It could still be an interesting aesthetic experience to keep unlocking new cards that are made up on the fly by an AI though, although not necessarily the one I'm primarily interested in). Either giving up on having infinite art or procedural generation are probably the way to go; the latter could be very rewarding, but extremely hard to get right and of course probably finite in the end anyway. In some hypothetical video game approaches, the player could be asked to draw the additional base cards, indices, and suits, or other things that might be required for arbitrary additional art needs, which I think would be pretty funny. Additionally the user would probably be provided a lame default of "King 2", etc.
...
Playing card games, in real life, are often played with an arbitrary number of decks shuffled together, and this number can be varied. That is, the cards can be drawn from one deck, which is reshuffled every time (or replaced with a fresh deck, or kept diminished without replacement until it is exhausted), or a "superdeck" can be created by shuffling together a certain number of decks of cards, which then basically acts like a standard deck except with n copies of each card, etc. This can be varied, and mostly affects the odds of various things happening, the ability to count cards, and also the ability to get new combinations impossible in a standard deck, like five-of-a-kind. There is also an interesting detail: it's an increasing hassle to use more and more decks, both in real life and in a simulation... until you get to infinity. Then it's impossible to do this in real life (or by the normal strategy of simulation — after all, it's impossible to shuffle an inifinite number of cards, or at least it would take an infinite amount of time), but the interpretation for a simulation is clear: you treat one normal deck as a set, from which you choose cards "with replacement", as they say in math. This is like you pick a card at random from the deck every time and then magically create a copy of it to deal (destroying that card after it's discarded), and then reshuffling the never-diminished "prototype" deck to pick the next card. You could even simulate this in real life, substituting "having a supply of each card ready" for "magic", although I don't think anyone has ever thought it worthwhile. However, this is actually easier to do in a computer than the normal deck simulation (albiet both are pretty trivial).
...
This graphical mnemonic establishes a correspondence between the numbers 1–5 and five suits. After the 5th suit the mnemonic breaks down pretty quickly, unless the 6th is hexagons (a powerful shape) or a flower with n pedals or a crown with n points etc, but at some point there's gonna have to be a suit symbol that isn't graphically related to a number like that, I think. Also, circles could be the 0th suit (0 points) or just another random suit; I've never been a huge fan of having a 0th element in things so I guess I wouldn't do that. I think _DDG_ uses circles for pips of a null suit / unsuited (which makes sense in that game because the suits have powers, so a null suit pip does normal stuff but with no bonus)(it would also make sense if you needed to match suits in some way, to have cards which would not match any suit (as a sort of obstacle)). Having this order 1–5 doesn't actually determine what beats what. Is 1 the first highest suit or the first lowest suit? For myself, I'm partial to the idea of quantity, so I think, eg, 4>3, and the fourth suit should beat the third suit. This has the added benefit that additional added suits will beat eariler suits, which probably affords slightly more exciting possibilities than if the new suits that are added are worse and worse. So, spades would be the lowliest suit (like a humble ditch-digger, perhaps) and hearts would beat it, and hearts would be beaten in turn by clubs, and so on.

There is, actually, an "etymological" answer to the objective order of suits, if we engage in a little speculative history, which is almost like our uniquely-determined ♠️♥️♣️♦️, by coincedence, although sadly backwards. You see, back when the Chinese were inventing their money cards, from which all cards seem to spread, the suits had an obvious numercal order, by amount of coins, as the suits were Coins, Strings of Coins, Myriads of coins (or maybe myriads of strings of coins — doesn't matter), and Tens of Myriads of coins. Obviously, these numbers go up! (Although, curiously, I'm told that in the Coins suit, lower numbers would win, for some reason.)

Now, with the order of suits determined obviously in chinese money cards, we can simply preserve this order as we progress through the historical evolution of playing card suits, according to speculation on Wikipedia pages that I skimmed, a small amount of unmarked guesswork/synthesis of my own:

* Chinese money cards: coins, strings of coins(look like sticks), myriads (万 "myriad" looks like a cup), tens of myriads (十 "ten" looks like a sword).
* Mahjong: coins, bamboo sticks, myriads. (tens of myriads suit is "stripped" from this play format)
* Mamluk cards: coins, polo-sticks, cups, swords.
* Latin cards: coins, batons, cups, swords.
* French cards: diamonds, clubs, hearts (cup-like drawing), spades.

Which leaves us with ♦️♣️♥️♠️. As stated before, it's neat that this is so similar to the "logical" order derived above, but inconvenient that this goes in exactly the opposite direction. Still, this gives us no particular way to continue the pattern logically. So maybe having this "etymological" order is not very useful. Also, as stated above, I'm tentatively considering allowing the various derived suits to coexist in the same deck, in which case perhaps it doesn't matter so much that in "my" ordering club is weaker than diamond, if (chinese?) coin is still weaker than string, as it obviously should be. Although... the possibility to have metasuits (latin, mamluk, chinese-money) which recapitulate the other structure is pretty pleasing, and could serve as a mnemonic... in fact, you could "unlock" the new suits by have the appropriate old suit "evolve" like a pokemon into the new ones. (Ironically you would start with the historically newest suits, french, as these are the most familiar (even globally), and unlock older and older suits as the "new" (to you) suits.)

The iconic [hanafuda](https://en.wikipedia.org/wiki/Hanafuda) cards are also ultimately derived from Western playing cards, specifically Portuguese-suited playing cards, which are latin, but since card-playing was illegal in various ways over time they disguised these cards so hard it's hard to say what the through-line is vis-à-vis suits or ranks. (The many Japanese variant playing cards created by this process are covered in some detail in https://en.wikipedia.org/wiki/Karuta). Are birds a kind of heart, somehow, for instance, or something like that? I doubt it! However, the swag of this merged [JACK OF RAIN MAIN](https://en.wikipedia.org/wiki/File:CaryJap2JackofSpades.jpg) is undeniable. Hanafuda suits also naturally have an order since they associated with the months. Their ranks also have a natural order given by the point values given by each rank in the most popular game(s) played with hanafuda, unless there are a lot more hanafuda games hiding somewhere of which I am completely ignorant.
...
The [Rider–Waite–Smith Tarot](https://en.wikipedia.org/wiki/Rider–Waite–Smith_Tarot) (rws) is a very nice tarot that has been very influential. It is latin-suited, and is largely used for mysticism. Its main infuriating idiosyncracy is swapping Justice and Strength (as is so often done in this modern world, har har) for its own perfidious reasons. I mean, presumably the deck still functions the same for actual games, which mostly just use the numerical value of the trump, but why would you ruin a perfectly good iclonis ordering like this?! (The answer is: spirituality reasons.) probably if featured in software there should be a toggle to swap back these numbers. It can be done. In fact, it should probably fixed (swapped back to the original tarot order) by default, and the toggle would let you enable the perfidy of RWS by swapping them back to RWS order. This involves editing the cards to change the numerals on them, in case that wasn't obvious.
...
Some Duke tobacco cards are already basically tarot cards, which is interesting. https://commons.wikimedia.org/wiki/Category:Cigarette_cards_published_by_W._Duke,_Sons_%26_Co._in_the_Metropolitan_Museum_of_Art#/media/File%3AA_great_big_bite%2C_from_the_Terrors_of_America_set_(N136)_issued_by_Duke_Sons_%26_Co._to_promote_Honest_Long_Cut_Tobacco_MET_DP865592.jpg
...
https://patents.google.com/patent/US6651984B1/en
